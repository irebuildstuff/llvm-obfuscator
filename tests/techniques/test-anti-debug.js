...
const config = TECHNIQUE_CONFIGS.antiDebug;

export async function testAntiDebug() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('anti-debug', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('anti-debug', config.webConfig);
