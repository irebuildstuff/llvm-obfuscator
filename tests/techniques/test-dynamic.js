...
const config = TECHNIQUE_CONFIGS.dynamic;

export async function testDynamic() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('dynamic', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('dynamic', config.webConfig);
