...
const config = TECHNIQUE_CONFIGS.virtualization;

export async function testVirtualization() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('virtualization', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('virtualization', config.webConfig);
