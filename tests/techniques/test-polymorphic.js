import { runCLIPipeline } from '../cli-runner.js';
import { runWebAppPipeline } from '../webapp-runner.js';
import { compareAllOutputs } from '../comparison.js';
import { TECHNIQUE_CONFIGS } from '../config.js';

const config = TECHNIQUE_CONFIGS.polymorphic;

export async function testPolymorphic() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('polymorphic', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('polymorphic', config.webConfig);
    const comparison = await compareAllOutputs(cliPaths, webappPaths);

    return {
      name: config.name,
      success: comparison.overall,
      comparison,
      cliPaths,
      webappPaths
    };
  } catch (error) {
    return {
      name: config.name,
      success: false,
      error: error.message
    };
  }
}


