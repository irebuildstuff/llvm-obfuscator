...
const config = TECHNIQUE_CONFIGS.fakeLoops;

export async function testFakeLoops() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('fake-loops', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('fake-loops', config.webConfig);
