...
const config = TECHNIQUE_CONFIGS.indirectCalls;

export async function testIndirectCalls() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('indirect-calls', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('indirect-calls', config.webConfig);
