...
const config = TECHNIQUE_CONFIGS.mba;

export async function testMBA() {
  console.log(`Testing: ${config.name}`);

  try {
    const cliPaths = await runCLIPipeline('mba', config.cliFlags);
    const { paths: webappPaths } = await runWebAppPipeline('mba', config.webConfig);
